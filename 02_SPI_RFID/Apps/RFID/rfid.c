...
//  rx_bits = MSS_SPI_transfer_frame( &g_mss_spi1, (((addr << 1) & 0x7E) << 8) |  val );
  //HAL_SPI_TransmitReceive(&spi1, (((addr << 1) & 0x7E) << 8) |  val , rx_bits, 1, 500);
  SPI_SendData(&spi1, &addr_bits, 1);
  SPI_SendData(&spi1, &val, 1);
  // clear the select line-- we are done here
//  MSS_SPI_clear_slave_select( &g_mss_spi1, MSS_SPI_SLAVE_0 );
  NSS_ENABLE();

  // burn some time
  // volatile uint32_t32_t ticks;
...
//  rx_bits = MSS_SPI_transfer_frame( &g_mss_spi1, ((((addr << 1) & 0x7E) | 0x80) << 8) | 0x00 );
  //HAL_SPI_TransmitReceive(&spi1, ((((addr << 1) & 0x7E) | 0x80) << 8) | 0x00 , rx_bits, 1, 500);
//HAL_SPI_Transmit(&spi1, (unsigned char*) ((((addr<<1) & 0x7E) | 0x80)), 1, 500);
	SPI_SendData(&spi1, &addr_bits, 1);

	SPI_RecieveData(&spi1, &rx_bits, 1);
  // clear the select line-- we are done here
//  MSS_SPI_clear_slave_select( &g_mss_spi1, MSS_SPI_SLAVE_0 );
